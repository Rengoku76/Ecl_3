...
class okno extends JFrame
{
	...
	private void btnClick(JButton btn) {
	 ...
	 if (oper==5)text.setText(""+(p1*p2/100));
	 if (oper==6)text.setText(""+(Math.pow(p1,p2)));
	 }
	 else if (str=="+")
	 {
		 oper = 1;
...
public okno()
{
    ...
    for (int i = 0;i<11;i++) btn[i].setText(""+i);
    btn[10].setText("+");
    btn[11].setText("-");
    btn[12].setText("/");
    btn[13].setText("*");
    ...
    btn[16].setText("%");
    btn[17].setText("^");
    btn[18].setText("Выход");
    JLabel lab = new JLabel ("Ответ:");
    lab.setFont(labFont);
